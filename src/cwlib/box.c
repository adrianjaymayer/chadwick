...
cw_box_process_boxscore_file(CWBoxscore *boxscore, CWGame *game)
{
  ...
  for (stat = game->first_stat; stat; stat = stat->next) {
    if (!strcmp(stat->data[0], "bline")) {
      slot = cw_atoi(stat->data[3]);
      team = cw_atoi(stat->data[2]);

      if (cw_atoi(stat->data[4]) == 1) {
	/* Record for starter */
	...
	boxscore->slots[slot][team] = player;
      ...
      player->batting->pa = -1;
      player->batting->ab = cw_atoi(stat->data[5]);
      player->batting->r = cw_atoi(stat->data[6]);
      boxscore->score[team] += player->batting->r;
      player->batting->h = cw_atoi(stat->data[7]);
      boxscore->hits[team] += player->batting->h;
      player->batting->b2 = cw_atoi(stat->data[8]);
      for (i = 1; i <= player->batting->b2; i++) {
	cw_box_add_event(&(boxscore->b2_list), -1, -1, 2,
			 player->player_id, "");
      }
      player->batting->b3 = cw_atoi(stat->data[9]);
      for (i = 1; i <= player->batting->b3; i++) {
	cw_box_add_event(&(boxscore->b3_list), -1, -1, 2,
			 player->player_id, "");
      }
      player->batting->hr = cw_atoi(stat->data[10]);
      for (i = 1; i <= player->batting->hr; i++) {
	cw_box_add_event(&(boxscore->hr_list), -1, -1, 2,
			 player->player_id, "");
      }
      player->batting->hrslam = -1;
      player->batting->bi = cw_atoi(stat->data[11]);
      player->batting->bi2out = -1;
      player->batting->sh = cw_atoi(stat->data[12]);
      for (i = 1; i <= player->batting->sh; i++) {
	cw_box_add_event(&(boxscore->sh_list), -1, -1, 2,
			 player->player_id, "");
      }
      player->batting->sf = cw_atoi(stat->data[13]);
      for (i = 1; i <= player->batting->sf; i++) {
	cw_box_add_event(&(boxscore->sf_list), -1, -1, 2,
			 player->player_id, "");
      }
      player->batting->hp = cw_atoi(stat->data[14]);
      player->batting->bb = cw_atoi(stat->data[15]);
      player->batting->ibb = cw_atoi(stat->data[16]);
      player->batting->so = cw_atoi(stat->data[17]);
      player->batting->sb = cw_atoi(stat->data[18]);
	...
	event->pickoff = -1;
      ...
      player->batting->cs = cw_atoi(stat->data[19]);
	...
	event->pickoff = -1;
      ...
      player->batting->gdp = cw_atoi(stat->data[20]);
      player->batting->xi = cw_atoi(stat->data[21]);
      player->batting->lisp = -1;
      player->batting->movedup = -1;
    }
    else if (!strcmp(stat->data[0], "pline")) {
      team = cw_atoi(stat->data[2]);
      seq = cw_atoi(stat->data[3]);
	...
	player->fielding[pos] = cw_box_fielding_create();
      ...
      player->fielding[pos]->outs = cw_atoi(stat->data[5]);
      player->fielding[pos]->po = cw_atoi(stat->data[6]);
      player->fielding[pos]->a = cw_atoi(stat->data[7]);
      player->fielding[pos]->e = cw_atoi(stat->data[8]);
      boxscore->errors[team] += player->fielding[pos]->e;
      player->fielding[pos]->dp = cw_atoi(stat->data[9]);
      player->fielding[pos]->tp = cw_atoi(stat->data[10]);
      player->fielding[pos]->pb = cw_atoi(stat->data[11]);
      player->fielding[pos]->bip = - 1;
      player->fielding[pos]->bf = -1;
      player->fielding[pos]->xi = -1;
    }
    else if (!strcmp(stat->data[0], "phline")) {
      team = cw_atoi(stat->data[3]);
      player = cw_box_find_player(boxscore, stat->data[1]);
      if (player == NULL) {
	fprintf(stderr,
	...
	exit(1);
      ...
      player->ph_inn = cw_atoi(stat->data[2]);
    }
	...
	exit(1);
      ...
      player->pr_inn = cw_atoi(stat->data[2]);
    }
    else if (!strcmp(stat->data[0], "tline")) {
      int team = cw_atoi(stat->data[1]);
      boxscore->lob[team] = cw_atoi(stat->data[2]);
      boxscore->er[team] = cw_atoi(stat->data[3]);
      boxscore->dp[team] = cw_atoi(stat->data[4]);
      boxscore->tp[team] = cw_atoi(stat->data[5]);
    }
  }

  for (stat = game->first_line; stat; stat = stat->next) {
    int team = cw_atoi(stat->data[0]);
    for (i = 1; i < stat->num_data; i++) {
      boxscore->linescore[i][team] = cw_atoi(stat->data[i]);
    }
  }
